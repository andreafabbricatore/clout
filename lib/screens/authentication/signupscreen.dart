...
class PhoneSignupScreen extends StatefulWidget {
  PhoneSignupScreen({super.key, required this.analytics});
                        ...
                        codeSent: (String verificationId, int? resendToken) {
                          try {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (_) => PhoneSignupOTPInputScreen(
...
class PhoneSignupOTPInputScreen extends StatefulWidget {
  PhoneSignupOTPInputScreen(
      {super.key,
  ...
  String referral;
  @override
  State<PhoneSignupOTPInputScreen> createState() =>
      _PhoneSignupOTPInputScreenState();
}

class _PhoneSignupOTPInputScreenState extends State<PhoneSignupOTPInputScreen> {
  db_conn db = db_conn();
        ...
        OtpTextField(
          ...
          onSubmit: (String verificationCode) async {
            ...
            await db.createuserinstance(
                usercredentials.user!.uid); //set all signup attributes to false
            if (widget.referral.isNotEmpty) {
              try {
                String shareruid = widget.referral.split("/").last;
                await db.referralcloutinc(usercredentials.user!.uid, shareruid);
                displayErrorSnackBar("Successfully referred!");
                gopicandnamescreen();
              } catch (e) {
                displayErrorSnackBar("Invalid code, change it or remove it.");
              }
            } else {
              gopicandnamescreen();
            }
